      ...
      <div className="balcao-shell">
        <aside className="balcao-sidebar">
          ...
          <div className="space-y-2">
            {[0, 1, 2].map((i) => (
              <div key={i} className="border rounded-lg p-3 bg-amber-50 cursor-pointer text-xs text-muted-foreground" onClick={() => setOpenSidebar(openSidebar === i ? null : i)}>
                <div className="flex items-center justify-between">
                  <span>O que fazer se um ficheiro nao abrir corretamente?</span>
                  <ChevronDown className={`w-3 h-3 shrink-0 transition-transform ${openSidebar === i ? "rotate-180" : ""}`} />
                </div>
                {openSidebar === i && <p className="mt-2">Deve tentar novamente :)</p>}
              </div>
            ))}
          </div>
        </aside>
        ...
        <main className="balcao-main">
          ...
          <h3 className="font-bold text-foreground mt-10 mb-4">Outros assuntos populares</h3>
          <div className="space-y-3">
            {mainFaqs.map((faq, i) => (
              <div key={i} className="bg-amber-50 rounded-lg overflow-hidden">
                <button onClick={() => setOpenMain(openMain === i ? null : i)} className="w-full flex items-center justify-between p-4 text-left font-medium text-foreground">
                  {faq.q}
                  <ChevronDown className={`w-4 h-4 shrink-0 transition-transform ${openMain === i ? "rotate-180" : ""}`} />
                </button>
                {openMain === i && (
